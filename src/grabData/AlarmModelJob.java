...
//    }
    ...
    2019.06.20 cjy: 修改告警方式，分两种，一种是原来的实时；另一种是事件分析告警
                    先读取alarmmethod 的值，0（原来的实时报警）；1（事件分析告警）
     */
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {List<EventPower> eventpowerlist = new ArrayList<>();
        ...
        try {
            ...
            bs = (BasicSetting)hbsessionDao.getFirst("From BasicSetting where id ='" + 0 + "'");
            Integer alarmmethod = bs.getAlarmmethod();

            if(alarmmethod == 0) { //0（原来的实时报警）
...
//////////////////////////以上为实时告警
            ...
            else if(alarmmethod == 1){ //1（事件分析告警）

                Integer alarmanalysisinterval = bs.getAlarmanalysisinterval();
                double alarmathreshold = bs.getAlarmanalysispercentthreshold();
                Integer datainterval = bs.getDatainterval();
                Integer tempinterval = bs.getTempinterval();
                double eventratioPower = 0.00;
                double eventratioTemp = 0.00;
...
//            HBSessionDaoImpl hbsessionDao = new HBSessionDaoImpl();
                ...
                Calendar calendar = Calendar.getInstance();
                calendar.setTime(new Date());
                calendar.add(Calendar.MINUTE, -alarmanalysisinterval);   // 让分钟减少alarmanalysisinterval
                String date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SS").format(calendar.getTime());
                ...
                //1.1.获得当前市行的alarmanalysisinterval分钟内的告警事件-电能事件
                //1.2.查询各个事件的告警方式、告警用户、告警时间范围
                //通过event_power.cid 找到 events_type中的type， 通过type 找到 device_threshold中的3条不同等级的记录，对应不同的tvalue[]
                //比较event_power.value 与 tvalue[] ，确定是哪一个等级的事件。tlevel
                //根据tlevel（0：不执行 1：弹窗 2：弹窗+短信/短信平台（工作日） 3.弹窗+短信/短信平台（全天））执行相应的告警
                ...
                Integer tlevel = 0;
                //读取当前时段（当前时刻到alarmanalysisinterval分钟前）的电能事件时，查询其中是否存在一条value=0且事件类型为上述事件类型时，舍弃其他事件，只报该条事件。
                eventpowerlist = hbsessionDao.search("FROM EventPower where time >'" + date + "' and signature is null");

                int powerevnnum = eventpowerlist.size();
                eventratioPower = powerevnnum/(alarmanalysisinterval*60)*datainterval;

                if(eventratioPower > alarmathreshold) {  // 大于百分比 报警
                    if (eventpowerlist != null) {
                        for (int i = 0; i < eventpowerlist.size(); i++) {
                            String did = eventpowerlist.get(i).getDid();
                            Integer cid = eventpowerlist.get(i).getCid();
                ...
                int tempevnnum = eventenvironmentlist.size();
                eventratioTemp = tempevnnum/(alarmanalysisinterval*60)*tempinterval;

                if(eventratioTemp > alarmathreshold) {  // 大于百分比 报警
                    if (eventenvironmentlist != null) {
                        for (int i = 0; i < eventenvironmentlist.size(); i++) {
                            String did = eventenvironmentlist.get(i).getDid();
                            Integer cid = eventenvironmentlist.get(i).getCid();
