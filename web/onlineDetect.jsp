...
<script type="text/javascript">
    ...
    function addData(newRecord) {
        // 构造1条记录
        // var time = newRecord["allRMS"][0]["time"];
        var time = new Date();
        // RMS
    ...
    // 清空qst echart数据
    function clearQSTdata() {
        qstRecordRmsu1 = [];
        qstRecordRmsu2 = [];
        qstRecordRmsu3 = []; // 暂存趋势图rmsu3数据
        qstRecordRmsv1 = [];// 暂存趋势图rmsu1数据
        qstRecordRmsv2 = []; // 暂存趋势图rmsu2数据
        qstRecordRmsv3 = []; // 暂存趋势图rmsu3数据
        qstRecordRmsi1 = []; // 暂存趋势图rmsi1数据
        qstRecordRmsi2 = []; // 暂存趋势图rmsi2数据
        qstRecordRmsi3 = []; // 暂存趋势图rmsi3数据
        qstRecordThdu1 = []; // 暂存趋势图thdu1数据
        qstRecordThdu2 = []; // 暂存趋势图thdu1数据
        qstRecordThdu3 = []; // 暂存趋势图thdu1数据
        qstRecordThdi1 = []; // 暂存趋势图thdi1数据
        qstRecordThdi2 = []; // 暂存趋势图thdi1数据
        qstRecordThdi3 = []; // 暂存趋势图thdi1数据
        qstRecordhz = []; // 暂存趋势图hz数据
        qstRecordp1 = []; // 暂存趋势图p1数据
        qstRecordp2 = []; // 暂存趋势图p2数据
        qstRecordp3 = []; // 暂存趋势图p3数据
        qstRecords1 = []; // 暂存趋势图s1数据
        qstRecords2 = []; // 暂存趋势图s2数据
        qstRecords3 = []; // 暂存趋势图s3数据
        qstRecordq1 = []; // 暂存趋势图q1数据
        qstRecordq2 = []; // 暂存趋势图q2数据
        qstRecordq3 = []; // 暂存趋势图q3数据
        qstRecordpf1 = []; // 暂存趋势图pf1数据
        qstRecordpf2 = []; // 暂存趋势图pf2数据
        qstRecordpf3 = []; // 暂存趋势图pf3数据
        qstRecordunb = []; // 暂存趋势图unb数据
        qstRecordpstu1 = []; // 暂存趋势图pstu1数据
        qstRecordpstu2 = []; // 暂存趋势图pstu2数据
        qstRecordpstu3 = []; // 暂存趋势图pstu3数据
        qstRecordpltu1 = []; // 暂存趋势图pltu1数据
        qstRecordpltu2 = []; // 暂存趋势图pltu2数据
        qstRecordpltu3 = []; // 暂存趋势图pltu3数据
            ...
            series: [
                {
                    ...
                    data: [{value: ((360 + data["angleU1"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FFFF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleU2"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#00FF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleU3"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FF0000'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleV1"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FFFF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleV2"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#00FF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleV3"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FF0000'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleI1"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FFFF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + 360 + data["angleI2"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#00FF00'
                        }
                ...
                },
                {
                    ...
                    data: [{value: ((360 + data["angleI3"]) % 360).toFixed(2)}],
                    itemStyle: {
                        normal: {
                            color: '#FF0000'
                        }
...
<script type="text/javascript">
    ...
    function updateParams(data, data2, data3) {
        ...
        $("#params-power").html(
            ...
            "<tr><th>PF</th><td>" + data["pf1"].toFixed(4) + "</td><td>" + data["pf2"].toFixed(4) + "</td><td>" + data["pf3"].toFixed(4) + "</td><td><tr>" + /*+Number(parseFloat(data["pf1"])+parseFloat(data["pf2"])+parseFloat(data["pf3"])).toFixed(2)+"</td></tr>"+*/
            "<tr><th>Cosφ</th><td>" + data["cosPhi1"].toFixed(4) + "</td><td>" + data["cosPhi2"].toFixed(4) + "</td><td>" + data["cosPhi3"].toFixed(4) + "</td><td></td></tr>" +
...
<script type="text/javascript">
    ...
    setInterval(function () {
        var computerroom = $("#comproom_code option:selected").val();
        var ctrlcname = $("#ctrlpnt").val();

        if (computerroom != "" && flag2 == 1) {
            getDefaultCtrlPoints();
            flag2 = 2;
        }
    }, 500);

    function getDefaultCtrlPoints() {
        var computerroom = $("#comproom_code option:selected").val();
        var ctrlname = $("#ctrlpnt").val();
        if (!ctrlname) {
            var hisvalue = $('#ctrlpnt').val();
            if (hisvalue == null) {
                $.ajax({
                    type: "post",
                    url: "getCtrlPoints",
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < rt.length; i++) {
                            if (i == 0)
                                $('#ctrlpnt').append("<option value='" + rt[i].did + "' selected='selected'>" + rt[i].name + "</option>");
                            else
                                $('#ctrlpnt').append("<option value='" + rt[i].did + "' >" + rt[i].name + "</option>");
                        }
                    }
                });
            }
    ...
    function updateCtrlData(data) {
        ...
        /* $("#ctrl-status").html(
             ...
             "<tr><th>电网电压异常</th><td>" + data["ctrl13"].toFixed(2) + "</td></tr>" +
             "<tr><th>软件欠压</th><td>" + data["ctrl14"].toFixed(2) + "</td></tr>" +
             "<tr><th>相序</th><td>" + data["ctrl15"].toFixed(2) + "</td></tr>" +
             "<tr><th>单边母线过压</th><td>" + data["ctrl16"].toFixed(2) + "</td></tr>" +
             "<tr><th>内部通信超时</th><td>" + data["ctrl17"].toFixed(2) + "</td></tr>" +
             "<tr><th>软件偏压</th><td>" + data["ctrl18"].toFixed(2) + "</td></tr>" +
             "<tr><th>外接CT异常</th><td>" + data["ctrl19"].toFixed(2) + "</td></tr>" +
             "<tr><th>断路器异常</th><td>" + data["ctrl20"].toFixed(2) + "</td></tr>" +

             "<tr><th>负载电流低</th><td>" + data["ctrl21"].toFixed(2) + "</td></tr>" +
             "<tr><th>超载保护</th><td>" + data["ctrl22"].toFixed(2) + "</td></tr>" +
             "<tr><th>内置CT异常</th><td>" + data["ctrl23"].toFixed(2) + "</td></tr>" +
             "<tr><th>远程闭锁</th><td>" + data["ctrl24"].toFixed(2) + "</td></tr>" +
             "<tr><th>IO扩展板通讯异常</th><td>" + data["ctrl25"].toFixed(2) + "</td></tr>" +
             "<tr><th>主从机指令通讯异常</th><td>" + data["ctrl26"].toFixed(2) + "</td></tr>" +
             "<tr><th>输出过流</th><td>" + data["ctrl27"].toFixed(2) + "</td></tr>" +
             "<tr><th>主从机双向通讯异常</th><td>" + data["ctrl28"].toFixed(2) + "</td></tr>" +
